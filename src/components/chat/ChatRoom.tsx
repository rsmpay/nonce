...
export function ChatRoom({ conversation, onBack }: ChatRoomProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 bg-surface border-b border-gray-800">
        <button
          onClick={onBack}
        ...
        <div className="relative" ref={menuRef}>
          ...
          {/* Dropdown Menu */}
          {showMenu && (
            <div className="absolute right-0 top-full mt-1 w-48 bg-surface-light border border-gray-700 rounded-xl shadow-lg overflow-hidden z-50">
              {canInvite() && (
                <button
                  onClick={() => {
                    setShowMenu(false);
                ...
                >
                  ...
                  <span className="text-white text-sm">초대 링크</span>
                </button>
              )}
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="text-white text-sm">멤버 목록</span>
              </button>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">첫 메시지를 보내보세요!</p>
          </div>
        ) : (
      ...
      {/* Image Preview */}
      {imagePreview && (
        <div className="px-4 py-2 bg-surface border-t border-gray-800">
      ...
      {/* Input */}
      {canSendMessage() ? (
        <div className="p-4 bg-surface border-t border-gray-800 safe-area-bottom">
          <div className="flex items-end gap-2">
            <button
              onClick={() => fileInputRef.current?.click()}
              className="p-3 hover:bg-surface-light rounded-full transition-colors flex-shrink-0"
            >
              <svg
                className="w-5 h-5 text-gray-400"
                fill="none"
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder="메시지 입력..."
                rows={1}
                className="w-full px-4 py-3 bg-surface-light border border-gray-700 rounded-2xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500 transition-colors resize-none max-h-32"
            ...
            <button
              ...
              className="p-3 bg-primary-600 hover:bg-primary-700 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-full transition-colors flex-shrink-0"
            >
              {sending ? (
                <svg
        ...
        <div className="p-4 bg-surface border-t border-gray-800 text-center text-gray-500 text-sm safe-area-bottom">
